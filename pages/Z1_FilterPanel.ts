...
export class FilterPanel {
  ...
  async fillCenaOd(cenaOd: number): Promise<void> {
    await this.root.getByRole('textbox', { name: 'priceFrom' }).fill(String(cenaOd)); // Fill mora da bude string
  }
  ...
  async selectStanje(stanja: readonly string[]): Promise<void> {
    // Iz fixtures.ts stanja[] je readonly string[]
